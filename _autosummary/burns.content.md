...
* [`content_aware_path()`](#burns.content.content_aware_path) — the **pure geometry**: given the image size and a
  *keep-region* (the union of face boxes if any, else a subject box), build a
  [`BurnsPath`](burns.path.md#burns.path.BurnsPath) whose start/end windows keep that region fully
...
[`pad_box()`](#burns.content.pad_box). [`burns.moves`](burns.moves.md#module-burns.moves) builds its static and drift moves out of the
...
| [`content_aware_path`](#burns.content.content_aware_path)(img_w, img_h, \*[, ...])        | A `BurnsPath` that keeps the subject/faces framed while zooming toward or away from them — the content-aware counterpart of [`ken_burns_path()`](burns.path.md#burns.path.ken_burns_path).                                         |
...
[`clamped()`](burns.rect.md#burns.rect.Rect.clamped)) — shrinking would change the window’s
