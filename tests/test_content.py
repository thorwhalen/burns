...
class TestContentAwarePath:
    def test_deterministic(self):
        ...
        assert content_aware_path(1600, 900, **kw) == content_aware_path(
            1600, 900, **kw
        )
    ...
    def test_mode_overrides_parity(self):
        s = (0.5, 0.5, 0.3, 0.3)
        assert (
            content_aware_path(1600, 900, subject=s, index=2, mode="in")
            .evaluate(1.0)
            .zoom
            >= 1.0
        )
    ...
    def test_keep_region_stays_framed_at_both_ends(self):
        ...
        p = content_aware_path(
            1920, 1080, subject=subject, index=1, output_aspect=16 / 9
        )
    ...
    def test_faces_take_priority_over_subject(self):
        ...
        p = content_aware_path(
            1600, 1200, subject=(0.8, 0.8, 0.1, 0.1), faces=faces, index=1
        )
    ...
    def test_window_matches_output_aspect(self):
        ...
        p = content_aware_path(
            iw, ih, subject=(0.4, 0.4, 0.3, 0.3), index=1, output_aspect=A
        )
        r = p.evaluate(0.0)
